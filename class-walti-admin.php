...
class Walti_Admin
{
	...
	public static function scan_page_load() {
		...
		if ( ! Walti_Credentials::isStored() ) {
			Walti_Flash::warning( sprintf( '組織の設定が完了していません。<br><a href="%s">設定ページ</a>からAPIキーとシークレットを登録してください。', self::getSettingPageUrl() ) );
			return;
		}
		...
		try {
			...
			if ( ! $target ) {
				Walti_Flash::warning( sprintf( 'このホストはターゲットとして登録されていません。<br><a href="%s">設定ページ</a>からターゲットの登録を行ってください。', self::getSettingPageUrl() ) );
				return;
			}

			if ( $target->isArchived() ) {
				Walti_Flash::warning( sprintf( 'このホストはアーカイブされています。<br>スキャンを実行する場合は<a href="%s" target="_blank">Walti.io</a>にログインしてターゲットを再度アクティブにしてください。', WALTI_URL . '/targets/' . Walti_Util::getHostName() ) );
				return;
			}
	 ...
	 *
	 */
	public static function schedule_page_load() {
		global $walti_exceptions;
		...
		if ( isset( $_POST['schedules'] ) ) {
			...
			try {
				$target = Walti_Target::fetch( $api, Walti_Util::getHostName() );
				if ( ! $target ) {
					throw new Exception( 'このホストはWalti.ioに登録されていません。設定画面からターゲット登録を行ってください。' );
				}
				if ( $target->isArchived() ) {
					throw new Exception( 'Walti.ioでこのターゲットがアーカイブされています。スケジュールを設定するにはWalti.ioにログインしてターゲットを再アクティベートしてください。' );
				}
...
EOS;
	...
	public static function render_option_page() {
		global $walti_exceptions;
		if ( ! empty( $walti_exceptions ) ) {
			Walti_Util::render( 'error', array( 'error' => $walti_exceptions[0] ) );
			return;
		}
		...
		if ( Walti_Credentials::isStored() ) {
			...
			try {
				...
				Walti_Logger::log( $e );
				Walti_Util::render( 'error', array( 'error' => $e ) );
				return;
			}
	...
	public static function render_schedule_page() {
		...
		try {
			...
			if ( $target->isArchived() ) {
				throw new Exception( 'Walti.ioでこのターゲットがアーカイブされています。スキャンを実行するにはWalti.ioにログインしてターゲットを再アクティベートしてください。' );
			}
		} catch ( Exception $e ) {
			Walti_Logger::log( $e );
			Walti_Util::render( 'error', array( 'error' => $e ) );
